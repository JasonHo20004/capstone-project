...
import type { RevenueOverviewResponse, RevenueByTransactionTypeResponse, RevenueByPeriodResponse } from "../dtos/revenue.dto";
...
export class AdminService {
  ...
  public async sendScheduledNotifications() {
    ...
    const totalTopupRevenue = transactions
      .filter((t) => t.transactionType === 'DEPOSIT')
      .reduce((sum, t) => sum + t.amount.toNumber(), 0);

    const totalSubscriptionRevenue = transactions
      .filter((t) => t.transactionType === 'PAYMENT' && t.subscriptionContractId !== null)
      .reduce((sum, t) => sum + t.amount.toNumber(), 0);

    const totalRevenue = totalTopupRevenue + totalSubscriptionRevenue;
  ...
  public async getRevenueByTransactionType(
    startDate?: string,
    endDate?: string,
    transactionType?: string
  ): Promise<RevenueByTransactionTypeResponse[]> {
    let dateFilter: any = {};
  ...
  public async getRevenueByPeriod(
    ...
    const totalRevenue = transactions.reduce((sum, t) => sum + t.amount.toNumber(), 0);
    const topupRevenue = transactions
      .filter((t) => t.transactionType === 'DEPOSIT')
      .reduce((sum, t) => sum + t.amount.toNumber(), 0);
