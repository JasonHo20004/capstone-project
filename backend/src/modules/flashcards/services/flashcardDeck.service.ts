...
export class FlashcardDeckService {
  ...
  public async getFlashcardDecks(userId: string): Promise<GetAllFlashcardDeckResponse[]> {
    
    const flashcardDecks =
      await this.flashcardDeckRepository.findFlashcardDeckByUserId(userId);
  ...
  public async updateFlashcardDeck(
    ...
    const updateFlashcardDeck = await this.flashcardDeckRepository.updateDeck(
      id,
      updatePayload,
      userId
    );
