    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries) {
          console.error("❌ [Identity Service] Database connection failed after all retries:", error);
          throw error;
        }
        const delay = baseDelayMs * Math.pow(2, attempt - 1) + Math.random() * 1000;
        console.warn(`⚠️ [Identity Service] Database connection attempt ${attempt}/${maxRetries} failed, retrying in ${Math.round(delay)}ms...`);
